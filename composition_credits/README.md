...
> musicos's canonical writing-credits (attribution) standard for musicos `Composition` objects.

**Attaches to:** `musicos::composition::Composition<CompositionShare>` via the object's cap-gated `uid_mut`. A single `CompositionCredits` record is stored as a dynamic field under the `ExtensionKey()` key, gated by `CompositionAdminCap<CompositionShare>`.
...
Added/removed events retain composition/admin-cap/party IDs, ordered compact role kinds (at most five), credit index/counts, and field lifecycle flags. Both are at most 128 BCS bytes. Display names and custom role labels stay in storage. The `Custom` tag remains distinct from canonical tags; the exact custom label requires reading the credit while present.
