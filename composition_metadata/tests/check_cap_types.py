...
for operation, args in (("set_title", ', b"Song".to_string()'), ("clear_title", "")):
    with tempfile.TemporaryDirectory(prefix="composition-metadata-cap-") as directory:
        root = Path(directory)
        ...
        (root / "sources" / "wrong_cap.move").write_text(f'''module composition_metadata::wrong_cap;
...
public fun rejected(comp: &mut Composition<SongA>, cap: &CompositionAdminCap<SongB>) {{
    composition_metadata::{operation}(comp, cap{args});
}}
''')
